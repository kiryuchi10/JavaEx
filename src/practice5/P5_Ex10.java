...
public class P5_Ex10 {
	...
	public static void main(String[] args) {
		...
		if (index >= 1 && index <= books.length) {
			books[index - 1].setprint(0); // Mark book as not available
			// System.out.println("Book with index " + index + " is now marked as not
			// available.");
		} else {
...
//		for (Book book : books) {
//			System.out.println(
//					"Index: " + book.getIndex() + ", Title: " + book.getTitle() + ", Author: " + book.getAuthor());
//		}

		for (int i = 0; i < book.length; i++) {
			String status = (book[i].getprint() == 0) ? "Not Available" : "Available";
			System.out.println("Index: " + book[i].getIndex() + ", Title: " + book[i].getTitle() + ", Author: "
					+ book[i].getAuthor() + ", Availability: " + status);
